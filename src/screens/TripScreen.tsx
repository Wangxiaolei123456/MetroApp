        ...
        {!active ? (
          <Card style={{alignItems: 'stretch'}}>
            <View style={{alignItems: 'center', paddingVertical: spacing.md}}>
              ...
              <Text style={{color: colors.textSub, marginTop: spacing.md, marginBottom: spacing.lg, textAlign: 'center', lineHeight: 20, fontSize: typography.sub}}>
                {t('trip.startHint')}
              </Text>
            </View>
            <Button title={t('trip.start')} onPress={handleStart} style={{marginHorizontal: 0, marginBottom: 0}} />
          ...
          <Card>
            <View style={styles.liveRow}>
              <View style={styles.liveRing}>
                <Text style={{fontSize: 36, fontWeight: '800', color: colors.primary}}>
                  {active.passedStations.filter((p) => p.valid).length - 1 < 0
                    ? 0
                    : active.passedStations.filter((p) => p.valid).length - 1}
                </Text>
              </View>
              <Text style={{color: colors.textSub, marginTop: spacing.sm, fontWeight: '600'}}>{t('trip.passedCount')}</Text>
            ...
            <View style={{marginTop: spacing.lg}}>
              {active.passedStations.map((p, i) => (
                <View key={p.stationId + p.enteredAt} style={styles.stationRow}>
                  <View style={{alignItems: 'center', width: 14}}>
                    <View style={[styles.dot, {backgroundColor: p.valid ? colors.success : colors.danger}]} />
                    {i < active.passedStations.length - 1 && <View style={styles.connector} />}
                  </View>
                  <Text style={{color: colors.text, flex: 1, fontWeight: '500'}}>{stationName(p.stationId)}</Text>
                  <Chip text={p.valid ? t('trip.valid') : t('trip.invalid')} color={p.valid ? colors.success : colors.danger} />
                </View>
              ))}
            </View>
            <Button title={t('trip.finish')} variant="danger" onPress={handleFinish} style={{marginHorizontal: 0, marginTop: spacing.md}} />
    ...
    <View style={{marginTop: spacing.md, paddingTop: spacing.md, borderTopWidth: 1, borderTopColor: colors.border}}>
      ...
      {trip.summary && calc && (
        <View style={{marginTop: spacing.sm}}>
          ...
          <Text style={{color: colors.textSub}}>{t('trip.reportDuration', {min: Math.round(trip.summary.durationMs / 60000)})}</Text>
          <Text style={{color: colors.primary, fontWeight: '700', marginTop: spacing.xs}}>
            {t('trip.reportPoints', {total: calc.total, base: calc.base, transfer: calc.transferBonus, long: calc.longBonus})}
          </Text>
        </View>
      )}
